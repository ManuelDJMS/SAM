     ...
     var obj = new Object();
      obj.Patron = $.trim($('#Patron').val());
       ...
       if(obj.Patron != '')
       {
          ...
          {
            alerta_error("INFORMACIÓN FALTANTE","Debe seleccionar un equipo/patrón");
          }
    });
  // =======================================================================
...
//----------CARGAR Y LIMPIAR LA TABLA DE USUARIO/SIGNATARIOS------------------
    function obtener_patrones(){
    var opc = "obtener_patrones";
    $('.line-scale-pulse-out').show();
    regenerar_tablaPatrones();
    $.post("dist/fw/agregarPatrones.php",{opc:opc},function(data){
        if(data){
            var html = '';
